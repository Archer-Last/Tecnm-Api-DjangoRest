...
#Se Cambio el nombre de la tabla "EstatusRequisitoAspirantes" para que concordara con el Modulo al que pertenece
class EstatusRequisitoAspirantes(models.Model):
...
class Documento(models.Model):
    ...
    detalle = models.ManyToManyField(Aspirante, through='DetalleDocumento')
...
def user_directory_path(instance, filename):
    ...
    return 'Alumnos/user_{0}_{1}/{2}'.format( re.sub(r"[^a-zA-Z0-9]","", str(instance.aspirante.Usuario)), re.sub(r"[^a-zA-Z0-9]","",str(instance.aspirante)), re.sub(r"[^a-zA-Z0-9]","",str(filename))+".pdf" )
...
class DetalleDocumento(models.Model):
    ...
    estatus = models.ForeignKey(EstatusRequisitoAspirantes, on_delete=models.CASCADE)
    def __str__(self):
        return str(self.aspirante) + str(self.requisito)


@receiver(post_delete, sender=DetalleDocumento)
...
@receiver(pre_save, sender=DetalleDocumento)
...
class Encuesta(models.Model):
    ...
    detalleentrevista= models.OneToOneField(DetalleEntrevista,on_delete=models.CASCADE, unique=True)
...
def update_requisito(sender, instance, **kwargs):
    ...
    if not my_objects:
         ...
         for aspirante in ListaAspirante:
              detalle = DetalleDocumento.objects.create(aspirante=aspirante,requisito=instance,estatus=statusR,observaciones=" ")
         detalle.save()     
...
def update_Aspirante(sender, instance, **kwargs):
     ...
     my_objects = list(DetalleDocumento.objects.filter(aspirante=instance))
     if not my_objects:
         statusR = EstatusRequisitoEstudiante.objects.get(pk=1)
    
         ListaRequisitos = Documento.objects.all()
         for requisitoN in ListaRequisitos:
              detalle = DetalleDocumento.objects.create(aspirante=instance,requisito=requisitoN,estatus=statusR,observaciones=" ")
         detalle.save()     
